<%@page import="in.co.rays.ORSProj3.model.RoleModelJDBCImpl"%>
<%@page import="in.co.rays.ORSProj3.controller.RoleListCtl"%>
...
<form action="<%=ORSView.ROLE_LIST_CTL%>" method="post" class="form-inline justify-content-center py-2">
...
<%=HTMLUtility.getList("rname", String.valueOf(dto.getId()), roleList) %>
</div>
</div>
</div>
...
<button type="submit" name="operation" value="<%=RoleListCtl.OP_SEARCH %>" class="btn btn-success text-uppercase mr-3 " style="position:static;">Search</button>
<button type="submit" name="operation" value="<%=RoleListCtl.OP_RESET %>" class="btn btn-warning text-uppercase px-3" style="position:static;">Reset</button>
</div>
...
<div class="col">
<%if (pageNo == 1) { %>
<button type="submit" name="operation" value="<%=RoleListCtl.OP_PREVIOUS %>" disabled class="btn btn-primary  btn-small ml-n3 text-uppercase">PREVIOUS</button>
<%} else { %>
<button type="submit" name="operation" value="<%=RoleListCtl.OP_PREVIOUS %>" class="btn btn-primary btn-small ml-n3 text-uppercase">PREVIOUS</button>
<% } %>
</div>

<div class="col text-center">
<button type="submit" name="operation" value="<%=RoleListCtl.OP_DELETE %>"  class="btn btn-small btn-danger mx-n3 text-uppercase">Delete</button>
</div>

<div class="col text-center">
<button type="submit" name="operation" value="<%=RoleListCtl.OP_NEW %>"  class="btn btn-small btn-success mx-n3 text-uppercase">New</button>
</div>

<% RoleModelJDBCImpl model = new RoleModelJDBCImpl(); %>
<div class="col text-right">
<%if (list.size() < pageSize || model.nextPK() -1 == dto.getId()) {%>
<button type="submit" name="operation" value="<%=RoleListCtl.OP_NEXT %>" disabled class="btn btn-primary btn-small ml-n3 text-uppercase">Next</button>
<% } else { %>
<button type="submit" name="operation" value="<%=RoleListCtl.OP_NEXT %>" class="btn btn-primary btn-small ml-n3 text-uppercase">Next</button>
<% } %>
</div>
...
<br>
<button type="submit" name="operation" value="<%=RoleListCtl.OP_BACK %>" class="btn btn-primary btn-small text-uppercase">Back</button>
<br>
</div>
<% } %>
</div>
